...
\STATE $t_e$ $\leftarrow$ $time_{cur} - time_{upload}$ \COMMENT{calculate week number of a requested video which is current time a requested video subtract by upload time of a requested video.}
\STATE $r_e$ $\leftarrow$ $viewcount_{cur} - viewcount_{lastweek}$ \COMMENT{calculate view rate of a requested video which is current view count of a requested video subtract by last week view count of a requested video.}
...
\begin{algorithm}
\caption{Determine phase for the first time access of a requested video}
\label{alg3}
\begin{algorithmic}[1]
...
\FOR{$i=0$ to $t_e$}
\STATE $total$ $\leftarrow$ $total$ + $count(d,i)$ \COMMENT{counting how many integer random number and sum those number}
\ENDFOR
...
Because of the very weak relationship between video size and popularity \cite{abhari2010workload} and because our work focuses on the impact of the popularity aspect on the utility function rather than storage optimization we believe that the choice to assign a random uniform video size from the Youtube dataset does not effect our results. 
...
\end{itemize}
Finally, we compare our results to PROP \cite{1613869}.
...
\begin{figure}[!t]
\begin{center}
\includegraphics[scale=0.7]{../../hindawi/graphs/new/repl/atd104.eps}
\end{center}
\caption{Comparison of available replicas between CPPro and PROP when a peer requests a video for scenario A with 100000 peers ($x$ axis in log-scale).}
\label{fig:atd-normal104}
\end{figure}

\begin{figure}[!t]
\begin{center}
\includegraphics[scale=0.7]{../../hindawi/graphs/new/shift/atd104.eps}
\end{center}
\caption{Comparison of available replicas between CPPro and PROP when a peer requests a video for scenario B with 100000 peers ($x$ axis in log-scale).}
\label{fig:atd-shift104}
\end{figure}
...
Figure~\ref{fig:contrib-normal}, \ref{fig:contrib-shift}, and \ref{fig:contrib-zipf} shows number of requested satisfied by peer.
...
Assume a peer requests a video with utility function $u_r$ and the minimum utility function inside the peer's cache is $u_c$-min.% as shown in fig.~\ref{fig:cached-notcached} where we have five videos inside the peer's cache.
Intuitively, when a requested video is in the after-peak phase and other videos in peer's storage are in the at-peak phase, then in the CPPro case there is a high probability that a requested video will not be cached.  
In the PROP case, since PROP does not use the video popularity phase, the probability of a requested video will not be cached is lower than CPPro.
We can take the same intuition for before-peak and at-peak phase.
...
Although, we can see two and three times fold difference in the head of distribution, in fact total difference is only $1\%$ for scenario A and B and $5\%$ for scenario C.
This is small difference because of small number of peers in the simulation which is same with number of available video.
The probability of different peers choose a same video is high here. 
We also performed simulation with 100000 peers and number of available replicas for scenario A and B are shown in fig.~\ref{fig:atd-normal104} and \ref{fig:atd-normal104}.
In case of 100000 peers, we can see the difference between CPPro and PROP in number of available replicas.
In this case, the probability of different peers choose a same video is low here. 
We calculate the Kolmogorov-Smirnov statistic on two samples and we find that for scenario A and scenario B the $p$-values are less than $0.1$ thus we consider the result are significant.

Figure~\ref{fig:duration-normal}, \ref{fig:duration-shift}, \ref{fig:duration-zipf} show cache duration from all scenarios.  
In scenario A and B, cache duration between PROP and CPPro are similar except in tail of distribution. 

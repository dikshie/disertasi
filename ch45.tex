...
\begin{equation}
\label{eq:rp}
rp_v(t) = \langle r_v(t), t_p(t) \rangle
\end{equation}
...
\STATE $t, r_v, t_p$ $\leftarrow$ read Borghol's dataset {where $t$ is week number (1st column), $r_v$ is view rate (2nd column), and $t_p$ is relative week to at-peak values (3rd column).}
...
\STATE $tp\_final$ $\leftarrow$ $average(t_p^{before}, t_p^{at}, t_p^{after})$
\IF{$tp\_final$ $<$ $0$} \COMMENT{if relative week to at-peak value is less than 0, estimate a requested video as before-peak.}
\STATE $phase$ $\leftarrow$ $before$
\ELSIF{$tp\_final$ $==$ $0$} \COMMENT{if relative week to at-peak value is equal to 0, estimate a requested video as at-peak.}
...
\FOR{$i=0$ to $t_e$}
\STATE $total$ $\leftarrow$ $total$ + $count(d,i)$ \COMMENT{counting how many integer random number and sum those}
\ENDFOR
...
The final view count is the total number of views of a video at the end of simulation and it is generated using uniform distribution from Borghol dataset.
The upload time interval is a Poisson process with $\lambda=1$.
The video size is generated using a uniform distribution.
Because of the very weak relationship betwen video size and popularity \cite{abhari2010workload} and because our work focuses on the impact of the popularity aspect on the utility function rather than storage optimization we believe that the choice to assign a random uniform video size from the Youtube dataset does not effect our results. 
